...
import { TrendingUp, TrendingDown, Package, IndianRupee, X, Calendar, User, MapPin, AlertTriangle, Clock, Share2, Calculator, CheckSquare, MessageSquare, Send, Bell, Phone, FileDown } from 'lucide-react';
...
function Dashboard({ currentUser }) {
  ...
  // Serial No. 3 — EOD Report generator (Admin only)
  // Uses rawData already loaded in memory — no extra Firestore reads needed.
  const generateEODReport = () => {
    try {
      const today = new Date();
      const todayDateStr = today.toDateString(); // used for .toDateString() comparison

      // Filter only today's Aavak entries with a valid netAmount
      const dailyAavak = rawData.aavak.filter(entry => {
        // billingDate is stored as "YYYY-MM-DD" — compare via toDateString on a parsed Date
        const entryDate = entry.billingDate
          ? new Date(entry.billingDate).toDateString()
          : null;
        const netAmt = parseFloat(entry.netAmount ?? entry.amountPaid ?? 0);
        return entryDate === todayDateStr && netAmt > 0;
      });

      // Filter only today's Javak entries
      const dailyJavak = rawData.javak.filter(entry => {
      ...
      doc.text('VCC COTTON - END OF DAY REPORT', 105, 18, { align: 'center' });

      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.text(`Date: ${dateLabel}`, 105, 26, { align: 'center' });
      doc.text(`Generated by: ${currentUser?.name || 'ADMIN'} at ${today.toLocaleTimeString('en-IN')}`, 105, 31, { align: 'center' });

      // ── Summary Box ─────────────────────────────────────────────────────────
      const todayAavakWtKg = dailyAavak.reduce((s, e) => s + parseFloat(e.netWt || 0), 0);
      const todayPayout    = dailyAavak.reduce((s, e) => s + parseFloat(e.netAmount ?? e.amountPaid ?? 0), 0);

      autoTable(doc, {
        startY: 37,
        head: [['Metric', 'Value']],
        body: [
          ['Today\'s Aavak Entries', dailyAavak.length],
          ['Today\'s Aavak Net Wt', `${todayAavakWtKg.toLocaleString('en-IN')} kg`],
          ['Today\'s Total Payout', `₹${todayPayout.toLocaleString('en-IN')}`],
          ['Today\'s Dispatch (Trucks)', dailyJavak.length],
          ['Cash Balance (Running)', `₹${cashBalance.toLocaleString('en-IN')}`],
        ],
        theme: 'grid',
      ...
      // ── Aavak Table ─────────────────────────────────────────────────────────
      const aavakY = doc.lastAutoTable.finalY + 10;
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(11);
      doc.text('AAVAK TRANSACTIONS (TODAY)', 14, aavakY);

      autoTable(doc, {
        startY: aavakY + 4,
        head: [['Token #', 'Farmer Name', 'Item', 'Net Wt (kg)', 'Amount (₹)', 'Payment Mode']],
        body: dailyAavak.length > 0
          ? dailyAavak.map(e => [
              e.tokenNo ?? '-',
              (e.Name || e.farmerName || 'UNKNOWN').toUpperCase(),
              (e.itemName || '-').toUpperCase(),
              parseFloat(e.netWt || 0).toLocaleString('en-IN'),
              parseFloat(e.netAmount ?? e.amountPaid ?? 0).toLocaleString('en-IN'),
      ...
      // ── Javak Table ─────────────────────────────────────────────────────────
      const javakY = doc.lastAutoTable.finalY + 10;
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(11);
      doc.text('JAVAK / DISPATCH (TODAY)', 14, javakY);

      autoTable(doc, {
        startY: javakY + 4,
        head: [['Gate Pass #', 'Vehicle No', 'Commodity', 'Destination', 'Net Wt (kg)', 'Bags']],
        body: dailyJavak.length > 0
          ? dailyJavak.map(e => [
              e.gatePassNo ?? '-',
              (e.vehicleNumber || '-').toUpperCase(),
              (e.commodity || '-').toUpperCase(),
              (e.destination || '-').toUpperCase(),
              parseFloat(e.netWt || 0).toLocaleString('en-IN'),
              parseInt(e.numberOfBags || 0, 10).toLocaleString('en-IN'),
            ])
          : [['—', 'No dispatches today', '', '', '', '']],
        theme: 'striped',
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(
          `VCC Cotton Management System • EOD Report • Page ${i} of ${pageCount}`,
          105, 290, { align: 'center' }
        );
      }

      const fileName = `EOD_Report_${today.toISOString().split('T')[0]}.pdf`;
      doc.save(fileName);

      setMessageStatus({ text: `EOD Report downloaded: ${fileName}`, type: 'success' });
      setTimeout(() => setMessageStatus({ text: '', type: '' }), 4000);

    } catch (err) {
      console.error('EOD PDF generation failed:', err);
      setMessageStatus({ text: 'Failed to generate EOD report. Please try again.', type: 'error' });
      setTimeout(() => setMessageStatus({ text: '', type: '' }), 4000);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          {/* Serial No. 3: EOD Report — visible to ADMIN only */}
          {isAdmin && (
            <button
              onClick={generateEODReport}
              className="flex items-center gap-2 whitespace-nowrap uppercase tracking-widest text-xs py-3 px-4 bg-indigo-700 hover:bg-indigo-800 active:scale-95 text-white font-bold rounded-xl transition-all shadow-md shadow-indigo-200 dark:shadow-none"
              title="Download End of Day PDF Report"
            >
              <FileDown className="w-4 h-4" /> EOD Report
            </button>
          )}
        </div>
      </div>
